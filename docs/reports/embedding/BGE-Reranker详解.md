...
>
> 本文写全 **BGE-Reranker 家族的四个版本**如何从「XLM-R Cross-Encoder」演进到「LLM-as-Reranker + 深度/宽度双压缩 + 自蒸馏 + 4 种 prompt 类型」。核心贡献是 **v2.5-gemma2-lightweight 的两级压缩机制**——在保住精度前提下把 FLOPs 砍到 40%，让 LLM 级精排也能上线。这份精排也是 [BGE-EN-ICL](BGE-EN-ICL详解.md) 与 [bge-multilingual-gemma2](BGE-multilingual-gemma2详解.md) 的蒸馏教师。
...
- 姊妹版本：[BGE-EN-ICL 详解](BGE-EN-ICL详解.md)（v2.5 是 EN-ICL 的蒸馏教师）· [bge-multilingual-gemma2 详解](BGE-multilingual-gemma2详解.md)（同样用 v2.5 蒸馏）
- 前置：[BGE-CPack 详解](BGE-CPack详解.md)（BGE 全家桶起点）· [BGE-M3 三功能统一详解报告](BGE-M3三功能统一详解报告.md)（v2-m3 出处）
- Late Interaction 对照：[ColBERT 详解](ColBERT详解.md) · [ColBERTv2 详解](ColBERTv2详解.md) · [ColPali 详解](ColPali详解.md) · [ColQwen 系列详解](ColQwen系列详解.md)
