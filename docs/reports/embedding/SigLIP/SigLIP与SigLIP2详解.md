...
- 前置：[CLIP 详解](../CLIP/CLIP详解.md)（SigLIP 只改损失和实现，双塔结构完全同源）。
- 双塔多模态路线全景：[图文 Embedding 模型技术综述](../图文Embedding模型技术综述.md)。
- 视觉文档多向量派（用 SigLIP 2 或 SigLIP-So400m 当视觉塔的一个变体）：[ColPali 详解](../ColPali/ColPali详解.md)、[ColQwen 系列详解](../ColQwen/ColQwen系列详解.md)。
- Jina-CLIP 系（CLIP/SigLIP 直接后续）：[jina-clip 系列详解](../Jina/clip/jina-clip系列详解.md)。
