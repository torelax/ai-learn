...
> **paper**：附录 C of [Making Text Embedders Few-Shot Learners (ICLR 2025)](https://arxiv.org/abs/2409.15700)（与 [BGE-EN-ICL 详解](EN-ICL/BGE-EN-ICL详解.md) 同篇论文的姊妹发布）
...
**理解顺序**：先读 [BGE-EN-ICL 详解](EN-ICL/BGE-EN-ICL详解.md)（拿到训练框架）→ 本篇（把英文换成多语场景）→ [BGE-M3 详解](M3/BGE-M3三功能统一详解报告.md)（多语的另一条路：多头统一 + 长上下文）。
...
- 姊妹版本：[BGE-EN-ICL 详解](EN-ICL/BGE-EN-ICL详解.md)（训练框架完全同源，本篇是多语版）
- 前置：[BGE-CPack 详解](C-Pack/BGE-CPack详解.md)（BGE 全家桶起点）· [BGE-M3 三功能统一详解报告](M3/BGE-M3三功能统一详解报告.md)（BAAI 多语传统）
