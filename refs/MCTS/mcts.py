...
class State:
    ...
    def reward(self):
        """reward 1- |value-goal|/MAX_VALUE \n
        总和越接近0 奖励越接近1， 反之奖励趋近于0"""
...
def UCTSEARCH(budget, root, num_moves_lambda=None):
    for iter in range(int(budget)):
        if iter % 10000 == 9999:
            ...
            logger.info(root)
        front = TREEPOLICY(root, num_moves_lambda) # exploration expand
        reward = DEFAULTPOLICY(front.state) # 执行一次rollout
        BACKUP(front, reward) # backup
    return BESTCHILD(root, 0)


def TREEPOLICY(node, num_moves_lambda):
    '''
    UCT一般形式是根据UCB公式select直到找到一个没有完全扩展的节点 \n
    如果有很多选项, 那么给定几率不进行扩展(exploration), 而是直接利用(exploitation)'''
